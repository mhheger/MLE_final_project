...
def setup_training(self):
    ...
    self.model.Inverse.to(device)
    self.model.Forward.to(device)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if SAVE_STATS:
        ...
        with open('stats.csv', 'a', newline='') as file:
            ...
            reset_stats(self)
    with open("ruediger-model.pt", "wb") as file:
...
def optimize_model(self, transition_batch):
    ...
    batch = random.choices(transition_batch,k= BATCH_SIZE)
    ...
    intrinsic_rewards = calc_intrinsic_rewards(self, states, actions, next_states).to(device)
    rewards = INTRINSIC_EXTRINSIC_RATIO * intrinsic_rewards + (1- INTRINSIC_EXTRINSIC_RATIO)* rewards
...
def calc_intrinsic_rewards(self, states, actions, next_states): 
    return optimize_intrinsic_models(self, states, actions, next_states)

def optimize_intrinsic_models(self, states, actions, next_states):
    actions = F.one_hot(actions, num_classes = 6).float().to(device)
    optimizer_inv = optim.Adam(self.model.Inverse.parameters(), lr = 2e-4)
    loss_fct_inv = nn.MSELoss()
    input_inv = torch.cat((states, next_states), dim = 1)
    pred_inv = F.softmax(self.model.Inverse(input_inv))
    loss_inv = loss_fct_inv(pred_inv, actions)
    optimizer_inv.zero_grad()
    loss_inv.backward()
    optimizer_inv.step()

    optimizer_forward = optim.Adam(self.model.Forward.parameters(), lr = 2e-4)
    loss_fct_forward = nn.MSELoss()
    input_forward = torch.cat((states, actions), dim = 1)
    pred_forward = self.model.Forward(input_forward)
    loss_forward = loss_fct_forward(pred_forward, next_states)
    optimizer_forward.zero_grad()
    loss_forward.backward()
    optimizer_forward.step()

    return (1- FORWARD_INVERSE_RATIO)*loss_inv + FORWARD_INVERSE_RATIO*loss_forward
